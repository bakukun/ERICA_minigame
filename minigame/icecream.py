...
def icecreamgame(money, ID, passwd, icecream_ok, roulette_ok, thief_ok):
    ...
    if (start == "Y" or start == "y"):
        ...
        if (money>=bat):
            ...
            icecream_main(money, ID, passwd, icecream_ok, roulette_ok, thief_ok, real_bat)
        else:
...
def icecreamgame2(money, ID, passwd, icecream_ok, roulette_ok, thief_ok):
    members = load_members()
    print("현재 ", money, "원 보유중입니다")
    while (True):  # 입력 예외처리
        try:
    ...
    if (money>=bat):
        ...
        icecream_main(money, ID, passwd, icecream_ok, roulette_ok, thief_ok, bat)
    else:
        print("\n당신의 보유금액으론 내기를 하지 못니다.\n도박은 가정파탄의 지름길 입니다.\n")
        print("배팅금액에 비해 정해진 기준의 잔고가 부족하여 게임을 종료합니다 다른 게임으로 돈을 벌고 오세요^0^")
        members[ID] = passwd, money, icecream_ok, roulette_ok, thief_ok
...
def icecreamregame(money, ID, passwd, icecream_ok, roulette_ok, thief_ok):
    members = load_members()
    ...
    if (restart == "Y" or restart == "y"):
        members[ID] = passwd, money, icecream_ok, roulette_ok, thief_ok
        store_members(members)
        icecreamgame2(money, ID, passwd, icecream_ok, roulette_ok, thief_ok)
    elif (restart == "N" or restart == "n"):
        members[ID] = passwd, money, icecream_ok, roulette_ok, thief_ok
        store_members(members)
        print("게임을 종료합니다")

def icecream_main(money, ID, passwd, icecream_ok, roulette_ok, thief_ok,real_bat):
    members = load_members()
    ment = "베스킨라빈스 31~~~\n"
    for k in ment:
        time.sleep(0.3)
    ...
    members[ID] = passwd, money, icecream_ok, roulette_ok, thief_ok
    store_members(members)
    icecreamregame(money, ID, passwd, icecream_ok, roulette_ok, thief_ok)